...
Verify that your retail game copy is fully up-to-date, through the official Launcher.
SPT requires that your retail game copy is on the latest version. This is so the downpatcher can patch them down to the same game client version that SPT 3.11 runs on.
...
> You CANNOT install SPT 3.11 using the Steam version of the game. You will need to install [SPT 4.0](https://wiki.sp-tarkov.com/Installation_Guide)
{.is-danger}
...
1. You will need to download a patcher to downgrade your game files. 
	- Each version of SPT requires a specific client version of the game. `SPT 3.11.4` requires `Game Client 16.1.3.35392`. The following downgrade is currently available:
	- [](PATCHER)[1.1.0.0.46624 to 16.1.3.35392](https://spt-legacy.modd.in/Patcher_1.1.0.0.46624_to_16.1.3.35392.7z)
...
3. Copy the content of your live game folder into your `SPT` folder.
	- **DON'T** delete the original retail game installation to save space, it must remain in the original install location for SPT to function.
...
9. Run `SPT.Launcher` and follow the onscreen instructions.
 - DO NOT COPY YOUR LIVE GAME SETTINGS. Copying your settings will result in the game hanging on load.
 	- If you copied your live game settings, go to the SPT Launcher Settings and click Clear Game Settings.
 - You can use any username you want. It is recommend that you **do not** use your retail account username. Especially if you plan on recording or streaming SPT.
 ...
 - Select your desired game version. Each version has a description box summarising what is included. Once you have picked your chosen game version click `Register`. You can pick *any* game version you want from the profile list, you do not need to own the corresponding game version. Once chosen, you cannot change the edition a profile is using.
